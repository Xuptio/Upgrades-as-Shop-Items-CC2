...
function render_map_facility_ui(screen_w, screen_h, x, y, w, h, category_data, facility_tile, screen_vehicle, is_active)
    ...
    ui:begin_window(category_data.name .. "##facility", 5, y, left_w - 5, h - 5, category_data.icon, is_windows_active and g_tab_map.selected_panel == 0, 2)
        --- if the island category is warehouse display the "inventory" row for selection
        ...
        --print('category_data.index',category_data.index)
        --print('screen_vehicle', screen_vehicle)
        function dump(o)
        ...
        --- these two if blocks populate the island production options, also set locked unlocked
        --- upgrades 
        if category_data.index == 0 then
    ...
    if g_tab_map.selected_facility_queue_item ~= -1 then
        --- this is the background of the queue popup window
        update_ui_rectangle(0, 0, screen_w, screen_h, color8(0, 0, 0, 200))
    ...
    if g_tab_map.selected_facility_item ~= -1 then
        ...
        --print('chosenitemtable', dump(chosenitemtable))
        --print('upgrade_1', dump(upgrade_1))

        --print('g_item_data[g_tab_map.selected_facility_item]', dump(g_item_data[g_tab_map.selected_facility_item]))
        --if upgradeclicked == true then
        --    print('user wants and upgrade ')
        --    item = upgrade_1
        --elseif upgradeclicked == false then
        --    item = g_item_data[g_tab_map.selected_facility_item]
        --end
        --print('g_tab_map.selected_facility_item', g_tab_map.selected_facility_item)
        if item ~= nil then
            -- check if user clicked upgrade item
            ...
            --local upgrade_selected 
            --if string.find(item['desc'], 'UPGRADE') then
            ...
            --local window = ui:begin_window(chosenitemtable["name"] .. "##facilityitem", 30, y + 10, w - 60, h - 30, atlas_icons.column_stock, true, 2)
            --print('item.name', item.name)
            --imgui_item_description(ui, chosenitemtable["desc"], chosenitemtable, false, true)
            --print('ui', dump(ui))
            --print('screen_vehicle', screen_vehicle)
            --print('item', dump(item))
            --print('ui', ui)
                
            -- add condition to catch upgrade clicked in facility 
            ...
            if result == 0 then
                ...
                print('I want one', 'credits', item['cost'], 'time', item['time'])
            elseif result == 1 then
            ...
            -- if these booleans are not reset here the upgrades will overwrite the 
            -- other selectable items in the table 
