...
function _update(screen_w, screen_h, ticks)
    ...
    render_shields_display(screen_w - 75, 5, g_tab_map.selected_facility_id ~= 0 and g_tab_map.selected_facility_item ~= -1)
...
function render_map_facility_ui(screen_w, screen_h, x, y, w, h, category_data, facility_tile, screen_vehicle, is_active)
    ...
    local left_w = w - 75
    local right_w = w - left_w
    --local chosenitemtable
    --local upgradeclicked
    -- upgrade 
    local upgrade_selection
    ...
    local window = ui:begin_window(category_data.name .. "##facility", 5, y, left_w - 5, h - 5, category_data.icon, is_windows_active and g_tab_map.selected_panel == 0, 2)
        ...
        if category_data.index == 0 then
            if team_data:get() then
                --local holdernumber = category_data.item_count + 2
                --print('category_data.item_count', category_data.item_count)
                --print('holdernumber', holdernumber)
                

                -- hijack blueprint table and add in custom data for upgrades to be shown
                table.insert(items_unlocked, upgrade_1)
                table.insert(items_unlocked, upgrade_2)
                --print(dump(g_item_data[e_inventory_item.support_turret_ciws]))
                for i = 1, category_data.item_count do
                    ...
                    if update_get_resource_item_hidden_facility_production(item.index) == false then
                        if team_data:get_is_blueprint_unlocked(item.index) then
                            ...
                            --table.insert(items_unlocked, g_item_data[e_inventory_item.virus_module])
                            --table.insert(items_unlocked, g_item_data[e_inventory_item.virus_module])
        ...
        if #items_unlocked > 0 then
            ...
            for i = 1, #items_unlocked do
                ...
                local is_action = imgui_table_entry(ui, columns, true)
                -- this logic captures if the user has clicked on an item in the facility
                -- production table
                -- need variable to capture that the user has clicked an upgrade
                -- this logic will be passed to the popup ui and 
                -- also the purchase of the upgrade

                if is_action then
                    g_tab_map.selected_facility_item = items_unlocked[i].index

                    -- this is the table of selection for the user 
                    -- it passes the index of the item to the selected facility item
                    
                    local strcheck = items_unlocked[i].name

                    -- upgrade selection is not being passed down
                    if string.find(strcheck, 'UPGRADE: Scouting Lv. 1') then
                        upgradescoutlv1 = true
                        upgrade = true
                        --print('upgrade after selection', upgrade)
                    --    upgrade_selection = upgrade_1
                    --    g_tab_map.selected_facility_item = 22

                        
                    elseif string.find(strcheck, 'UPGRADE: Scouting Lv. 2') then
                        upgradescoutlv2 = true
                        upgrade = true
                    --    upgrade_selection = upgrade_2
                    --    g_tab_map.selected_facility_item = 22
    ...
    -- this is the sub ui that shows the status of the facotry idle, or building
    ...
    -- this is the sub ui that shows the status of the facotry queue
    ...
    -- this is the interaction with the ui popup when clicking on the queue to cancel items
    ...
    -- this is the loop for displaying the pop up menu after selecting the items from the home facility 
    -- basically the index of the item selected is passed to the popup ui and populated 
    -- with the data obtained from the items index and the associated description 
    -- we can hijack the data table to display our own data table with logic flags
    -- that capture if the user has clicked an upgrade
    if g_tab_map.selected_facility_item ~= -1 then
        update_ui_rectangle(0, 0, screen_w, screen_h, color8(0, 0, 0, 200))
        local item = g_item_data[g_tab_map.selected_facility_item]

        if upgradescoutlv1 == true then
            chosenitemtable = upgrade_1
            --upgrade = true
        elseif upgradescoutlv2 == true then
            chosenitemtable = upgrade_2
            --upgrade = true
        ...
        --local drydock_island_id = find_team_drydock(team)
        --local shield_count_l = get_team_shield_count(drydock_island_id)
        --print('add 2 shields to count...')
        --set_team_shield_count(drydock_island_id, 2)


        if item ~= nil then
            
            -- hijack and populate the pop up ui with our custom upgrade details 
            if upgradescoutlv1 == true then
                item['name'] = chosenitemtable['name']
                item['time'] = chosenitemtable['time']
            ...
            if upgrade then
                imgui_item_description_upgrade(ui, screen_vehicle, item, false, true)
            ...
            if upgrade then
                ...
                imgui_table_header(ui, {
                    { w=134, margin=5, value="Installed" },
                    { w=10, margin=0, value=atlas_icons.hud_warning },
                    { w=-1, margin=0, value="x" .. tostring(isavailable) }
                })
                ui:spacer(1)
                --print("item['purchased']", item['purchased'])
                --print('scouting_lv1_purchased', scouting_lv1_purchased)
                local result = ui:button_group({ "Purchase Upgrade -" .. tostring(item['shieldcost']) }, true)
                
                --print('upgrade flag', get_upgrade_flag(drydock_island_id, item['flag_ref']))

                --print('enter max upgrade amount here')
                --if item['purchased'] == 0 then 
                --    local result = ui:button_group({ "Purchase Upgrade -" .. tostring(item['shieldcost']) }, true)
                --elseif item['purchased'] == 1 then
                --    local result = ui:button_group({ "Purchase Upgrade -" .. tostring(item['shieldcost']) }, false)
                --end
                ...
                if result == 0 and shield_count_l >= item['shieldcost'] and item['purchased'] < 1 then
                    ...
                    --print('shield_count_l 3', shield_count_l)
                    --g_shield_count.shield_count = shield_count_l

                    --print('get shields', get_shield_count(g_shield_count))
            ...
            upgradescoutlv1 = false
            upgradescoutlv2 = false
        else
            --upgradescoutlv1 = false
            --upgradescoutlv2 = false
            g_tab_map.selected_facility_item = -1
...
-- shileds not updating on both displays... 
function render_shields_display(x, y, is_active)
                                --shield_count
                               -- )
    ...
    --    myfile=ingest("test_upgrades.lua")
    --    print(myfile)
        --print('error', status, err)
        --print('island:get_turret_spawn(turret_spawn_index)', island:get_turret_spawn(turret_spawn_index))
                    
    --    end)
    --print(status, err)
    

    local col = iff(is_active, iff(shields > 0, color_status_ok, color_status_bad), color_grey_dark)
