    ...
    if vehicle:get() then
        if g_is_connected then
            ...
            if not is_render_awacs and not g_is_map_overlay then
                ...
                --- smallx and smally are the 
                local smallx = 3 
                local smally = 2  
                local divfactor = 5
                local halfdivfactor = divfactor/2
                ...
                render_map_details_minimap(map_x, map_y, map_w, map_h, screen_w, screen_h, vehicle, attachment, 
                --(screen_w/4)- smallx+(screen_w/8) , (screen_h/4)- smally+(screen_h/8) 
                --map_w+(map_w/(halfdivfactor)) , map_h+(map_h/(halfdivfactor)) 
...
function render_map_details_minimap(x, y, w, h, screen_w, screen_h, screen_vehicle, attachment, offset_x, offset_y)
    ...
    local camera_size = 10000 --10000
    local world_offx = (offset_x*10*3.9)--camera_size/2 -- looks ok at 800 x 600 
    local world_offy = (offset_y*10*3.9)--camera_size/3 -- look ok at 600 
    --print('world_offx', world_offx)
    --print('world_offy', world_offy)
    --print('screen_w', screen_w,'screen_h', screen_h)
    --print('offset_x', offset_x,'offset_y', offset_y)
    --print('ratio w/h ', screen_w/screen_h)
    --print('ratio h/w ', screen_h/screen_w)
    --print('camera_x', camera_x, 'camera_y', camera_y)

    --local scx_offx = screen_x + offset_x
    --local scy_offy = screen_y+ offset_y
    --local world_scx_offx, world_scy_offy = world_to_screen(scx_offx:x()  , scy_offy:y() )
    --update_set_screen_map_position_scale(camera_x - world_scx_offx, camera_y + world_scy_offy , camera_size)
    ...
    local is_render_islands = (camera_size < (64 * 1024))
    --print('camera_size',camera_size)
    --print('64 * 1024',64 * 1024)
    --print('is_render_islands',is_render_islands)
    --update_set_screen_background_is_render_islands(is_render_islands)
    ...
    local team_color = color_friendly
    local screen_x, screen_y = world_to_screen(camera_x, camera_y)
    --print('camscreen_x', screen_x,'camscreen_y', screen_y)
    --print('camscreen_x + offset_x', screen_x+offset_x, 'camscreen_y + offset_y', screen_y+offset_y)
    --update_set_screen_map_position_scale(camera_x + world_offx , camera_y- world_offy, camera_size)
    ...
    for _, vehicle in iter_vehicles(filter_vehicles) do
        local icon_region, icon_offset = get_icon_data_by_definition_index(vehicle:get_definition_index())
        --print('icon_region', icon_region)
        --print('icon_offset', icon_offset)
        local position_xz = vehicle:get_position()
        local screen_x, screen_y = world_to_screen(position_xz:x(), position_xz:z())
        local vehicle_team = vehicle:get_team_id()
        ...
        --print('math.floor(screen_w/2), math.floor(screen_h/2)', math.floor(screen_w/2), math.floor(screen_h/2))
        ...
        if  screen_x <= math.floor(screen_w/2) +1 and screen_x >= math.floor(screen_w/2) -1 and screen_y <= math.floor(screen_h/2) +1 and screen_y >= math.floor(screen_h/2) -1 then 
            --print('bingo')
            --print('vehicle', vehicle:get_id())
            controlling_vehicle_id = vehicle:get_id()
        end

        local element_color = color8(16, 16, 16, 255)
        --print('element_color.red',element_color.colors)
        --local element_color = color8(32, 32, 32, 255)

        if vehicle_team == update_get_screen_team_id() then
            element_color = color_friendly
            --print('update_get_team_color',  update_get_screen_team_id())
        else
            element_color = color_enemy -- color8(0, 0, 0, -100)
            --print('color_enemy.alpha',color_enemy.alpha )
            --element_color.a
        end
        ...
        if vehicle:get_is_visible() and vehicle:get_id() ~= controlling_vehicle_id then
            ...
            --print('vehicle', vehicle:get_definition_index())
